...
#define RIGHT(x) ((x)*2+2)
    ...
    inline void update_max(int x) {
      ...
      //func[x] =   x >= first ? (isum[x]>0?weight[x]:0) : (isum[x] >0?weight[x] : func[LEFT(x)]+func[RIGHT(x)]);
      func[x] =   x >= first ? isum[x]*weight[x] : isum[x] *weight[x] +func[LEFT(x)]+func[RIGHT(x)];
    }

    // This is used for precomputed value agregation to result
    inline void AG1(int &result, int node) {
      result +=propsum[node]*weight[node]+func[node];
    }
