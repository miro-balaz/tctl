...
string a;
REP(i,ratings.size()) a.append(ratings[i]);
istringstream is(a);
vector<double> f;
double c;
while(is>>c) f.push_back(c);
int R=(f.size()+19)/20;
double ellys=f[0];
sort(f.begin(), f.end(), greater<double>());
double sum=0;
double count=0;
double prob_bigger=0;
double sum_bigger=0;
for(unsigned int i=0;i<f.size();i+=R) {

 unsigned int limit=min<unsigned int>(i+R,f.size());
 if(limit==i+R) {
   count++;
   double s=0;
   for(unsigned int j=i;j<limit;++j) s+=f[j];
   s/=(double)R;
   for(unsigned int j=i;j<limit;++j) if(f[j]==ellys)s=ellys;
   sum+=s;
 } else {
   int is_ellys=0;
   for(unsigned int j=i;j<limit;++j) if(f[j]==ellys) is_ellys=1;
   double s=0;
   for(unsigned int j=i;j<limit;++j) s+=f[j];
   if(is_ellys) {
     prob_bigger=1.0;
     ...
     int count_last = limit-i;
    sum_bigger = s/double(count_last);
     prob_bigger = double(count_last)/(double)R;
   }
 }

}
return (sum/count)*(1-prob_bigger)+(prob_bigger)*((sum+sum_bigger)/(count+1));
